...
iter, VOCAB_SIZE = process_data(batch_size, SKIP_STEP)
...
def word2vec(batch_gen):
    ...
    with tf.name_scope('loss'):
        embed = tf.nn.embedding_lookup(
            embed_matrix, center_words, name='embed')
    ...
    norms = tf.sqrt(tf.reduce_sum(
        tf.square(embed_matrix), 1, keepdims=True))
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter('./log/graphs/no_frills/', sess.graph)
        ...
        gensim_model = GensimWrapper(EMBED_SIZE, iter=0)
        ...
        vocab = list(int2word.keys())
        gensim_model.set_embeddings(word2int, final_embeddings)
        # gensim_model = gensim.models.Word2Vec(vectors=final_embeddings, vocab=vocab)
        gensim_model.evaluate()
        writer.close()


def main():
    gen = generate_batch_v2(BATCH_SIZE, SKIP_WINDOW)
